...
const iconGoals = ['img/shorty.svg', 'img/longjohns.svg', 'img/tshirt.svg', 'img/jeans.svg', 'img/jacket.svg', 'img/slush.svg', 'img/clouds.svg', 'img/windy.svg'];

// icons
const blueIcon = L.divIcon({className: 'blue-icon'});
const greenIcon = L.divIcon({className: 'green-icon'});

// form for player name + check for letters only
document.querySelector('#player-form').addEventListener('submit', function (evt) {
    evt.preventDefault();
    const playerName = document.querySelector('#player-input').value;
    let letters = /^[A-Za-z]+$/;
    if (document.querySelector('#player-input').value.match(letters)) {
    document.querySelector('#player-modal').classList.add('hide');
    gameSetup(`${apiUrl}newgame?player=${playerName}&loc=${startLoc}`);
    }
    else {
        alert("Invalid player name. Use letters only!")
    }
});
